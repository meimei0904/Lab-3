/////*
//// * fsm_automatic.c
//// *
//// *  Created on: Nov 1, 2024
//// *      Author: MY LE
//// */
////
//#include "fsm_automatic.h"
//
//enum LEDState {RED, YELLOW, GREEN};
//
//int count11 = RED_INIT;
//int count22 = GREEN_INIT;
//enum LEDState state11 = RED;
//enum LEDState state22 = GREEN;
//
//void NormalMode() {
//	count11--;
//	count22--;
//	switch (state11) {
//		case RED:
//			HAL_GPIO_WritePin(R1_GPIO_Port, R1_Pin, GPIO_PIN_SET);
//			HAL_GPIO_WritePin(Y1_GPIO_Port, Y1_Pin, GPIO_PIN_RESET);
//			HAL_GPIO_WritePin(G1_GPIO_Port, G1_Pin, GPIO_PIN_RESET);
//			if (count11 < 0) {
//				count11 = green_value - 1;
//				state11 = GREEN;
//				HAL_GPIO_WritePin(R1_GPIO_Port, R1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(Y1_GPIO_Port, Y1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(G1_GPIO_Port, G1_Pin, GPIO_PIN_SET);
//			}
//			break;
//		case YELLOW:
//			HAL_GPIO_WritePin(R1_GPIO_Port, R1_Pin, GPIO_PIN_RESET);
//			HAL_GPIO_WritePin(Y1_GPIO_Port, Y1_Pin, GPIO_PIN_SET);
//			HAL_GPIO_WritePin(G1_GPIO_Port, G1_Pin, GPIO_PIN_RESET);
//			if (count11 < 0) {
//				count11 = red_value - 1;
//				state11 = RED;
//				HAL_GPIO_WritePin(R1_GPIO_Port, R1_Pin, GPIO_PIN_SET);
//				HAL_GPIO_WritePin(Y1_GPIO_Port, Y1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(G1_GPIO_Port, G1_Pin, GPIO_PIN_RESET);
//			}
//			break;
//		case GREEN:
//			HAL_GPIO_WritePin(R1_GPIO_Port, R1_Pin, GPIO_PIN_RESET);
//			HAL_GPIO_WritePin(Y1_GPIO_Port, Y1_Pin, GPIO_PIN_RESET);
//			HAL_GPIO_WritePin(G1_GPIO_Port, G1_Pin, GPIO_PIN_SET);
//			if (count11 < 0) {
//				count11 = yellow_value - 1;
//				state11 = YELLOW;
//				HAL_GPIO_WritePin(R1_GPIO_Port, R1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(Y1_GPIO_Port, Y1_Pin, GPIO_PIN_SET);
//				HAL_GPIO_WritePin(G1_GPIO_Port, G1_Pin, GPIO_PIN_RESET);
//			}
//			break;
//		default:
//			break;
//	}
//	switch (state22) {
//		case RED:
//			HAL_GPIO_WritePin(R2_GPIO_Port, R2_Pin, GPIO_PIN_SET);
//			HAL_GPIO_WritePin(Y2_GPIO_Port, Y2_Pin, GPIO_PIN_RESET);
//			HAL_GPIO_WritePin(G2_GPIO_Port, G2_Pin, GPIO_PIN_RESET);
//			if (count22 < 0) {
//				count22 = green_value - 1;
//				state22 = GREEN;
//				HAL_GPIO_WritePin(R2_GPIO_Port, R2_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(Y2_GPIO_Port, Y2_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(G2_GPIO_Port, G2_Pin, GPIO_PIN_SET);
//			}
//			break;
//		case YELLOW:
//			HAL_GPIO_WritePin(R2_GPIO_Port, R2_Pin, GPIO_PIN_RESET);
//			HAL_GPIO_WritePin(Y2_GPIO_Port, Y2_Pin, GPIO_PIN_SET);
//			HAL_GPIO_WritePin(G2_GPIO_Port, G2_Pin, GPIO_PIN_RESET);
//			if (count22 < 0) {
//				count22 = red_value - 1;
//				state22 = RED;
//				HAL_GPIO_WritePin(R2_GPIO_Port, R2_Pin, GPIO_PIN_SET);
//				HAL_GPIO_WritePin(Y2_GPIO_Port, Y2_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(G2_GPIO_Port, G2_Pin, GPIO_PIN_RESET);
//			}
//			break;
//		case GREEN:
//			HAL_GPIO_WritePin(R2_GPIO_Port, R2_Pin, GPIO_PIN_RESET);
//			HAL_GPIO_WritePin(Y2_GPIO_Port, Y2_Pin, GPIO_PIN_RESET);
//			HAL_GPIO_WritePin(G2_GPIO_Port, G2_Pin, GPIO_PIN_SET);
//			if (count22 < 0) {
//				count22 = yellow_value - 1;
//				state22 = YELLOW;
//				HAL_GPIO_WritePin(R2_GPIO_Port, R2_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(Y2_GPIO_Port, Y2_Pin, GPIO_PIN_SET);
//				HAL_GPIO_WritePin(G2_GPIO_Port, G2_Pin, GPIO_PIN_RESET);
//			}
//			break;
//		default:
//			break;
//	}
//
//}
//
//void LedDispMode() {
//	switch (mode) {
//		case 1:
//			if (get_timer1_flag()) {
//				setTimer1(250);
//				NormalMode();
//			}
//			break;
//		case 2:
//			if (get_timer1_flag()) {
//				setTimer1(200);
//				HAL_GPIO_TogglePin(R1_GPIO_Port, R1_Pin);
//				HAL_GPIO_TogglePin(R2_GPIO_Port, R2_Pin);
//				HAL_GPIO_WritePin(Y1_GPIO_Port, Y1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(G1_GPIO_Port, G1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(Y2_GPIO_Port, Y2_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(G2_GPIO_Port, G2_Pin, GPIO_PIN_RESET);
//			}
//			break;
//		case 3:
//			if (get_timer1_flag()) {
//				setTimer1(200);
//				HAL_GPIO_TogglePin(Y1_GPIO_Port, Y1_Pin);
//				HAL_GPIO_TogglePin(Y2_GPIO_Port, Y2_Pin);
//				HAL_GPIO_WritePin(R1_GPIO_Port, R1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(G1_GPIO_Port, G1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(R2_GPIO_Port, R2_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(G2_GPIO_Port, G2_Pin, GPIO_PIN_RESET);
//			}
//			break;
//		case 4:
//			if (get_timer1_flag()) {
//				setTimer1(200);
//				HAL_GPIO_TogglePin(G1_GPIO_Port, G1_Pin);
//				HAL_GPIO_TogglePin(G2_GPIO_Port, G2_Pin);
//				HAL_GPIO_WritePin(Y1_GPIO_Port, Y1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(R1_GPIO_Port, R1_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(Y2_GPIO_Port, Y2_Pin, GPIO_PIN_RESET);
//				HAL_GPIO_WritePin(R2_GPIO_Port, R2_Pin, GPIO_PIN_RESET);
//			}
//			break;
//		default:
//			break;
//	}
//}
